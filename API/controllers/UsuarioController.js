...
class UsuarioController {
	...
	store(req, res, next) {
		...
		if (!nome || !email || !password) {
			return res.status(422).json({ errors: "Preencha todos os campos de cadsatro." });
		}
	...
	createRecovery(req, res, next) {
		...
		if (!email) {
			return res.render('recovery', { error: "Preencha com o seu email cadastrado", success: null });
		}

		Usuario.findOne({ email }).then((usuario) => {
			if (!usuario) {
				return res.render('recovery', { error: "Não existe usuário com este email", success: null });
			}
	...
	showCompleteRecovery(req, res, next) {
		...
		Usuario.findOne({ "recovery.token": req.query.token }).then(usuario => {
			if (!usuario) {
				return res.render('recovery', { error: "Não existe usuário com este token", success: null });
			}
	...
	completeRecovery(req, res, next) {
		const { token, password } = req.bady;
		if (!token || !password) {
			return res.render("recovery/store", { error: "Preencha novamente a sua nova senha", success: null, token: token });
		}
